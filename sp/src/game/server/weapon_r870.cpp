...
class CWeaponRemington : public CBaseHLCombatWeapon
{
	DECLARE_DATADESC();
public:
	DECLARE_CLASS(CWeaponRemington, CBaseHLCombatWeapon);
...
IMPLEMENT_SERVERCLASS_ST(CWeaponRemington, DT_WeaponRemington)
END_SEND_TABLE()

LINK_ENTITY_TO_CLASS(weapon_r870, CWeaponRemington);
PRECACHE_WEAPON_REGISTER(weapon_r870);
...
//-----------------------------------------------------------------------------
float CWeaponRemington::GetMinRestTime()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
float CWeaponRemington::GetMaxRestTime()
{
	if (hl2_episodic.GetBool() && GetOwner() && GetOwner()->Classify() == CLASS_COMBINE)
	{
		return 1.5f;
...
//-----------------------------------------------------------------------------
float CWeaponRemington::GetFireRate()
{
	if (hl2_episodic.GetBool() && GetOwner() && GetOwner()->Classify() == CLASS_COMBINE)
	{
		return 0.8f;
