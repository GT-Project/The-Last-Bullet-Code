...
//---------------------------------------------------------------------------Ч 
// обед, лечит игрока при подн€тии 
//---------------------------------------------------------------------------Ч 
class CHotlunch : public CItem
{
public:
	DECLARE_CLASS(CHotlunch, CItem);
...
LINK_ENTITY_TO_CLASS(item_hotlunch, CHotlunch);
PRECACHE_REGISTER(item_hotlunch);


//---------------------------------------------------------------------------Ч 
// Purpose: 
//---------------------------------------------------------------------------Ч 
void CHotlunch::Spawn(void)
{
	Precache();
	SetModel("models\\newfood\\hotlunch\\hotlunch.mdl");
...
// Purpose: 
//---------------------------------------------------------------------------Ч 
void CHotlunch::Precache(void)
{
	PrecacheModel("models\\newfood\\hotlunch\\hotlunch.mdl");

	PrecacheScriptSound("Hotlunch.Touch");
}
...
// Output : 
//---------------------------------------------------------------------------Ч 
bool CHotlunch::MyTouch(CBasePlayer *pPlayer)
{
	if (pPlayer->TakeHealth(sk_hotlunch.GetFloat(), DMG_GENERIC))
	{
		CSingleUserRecipientFilter user(pPlayer);
		user.MakeReliable();
		...
		CPASAttenuationFilter filter(pPlayer, "Hotlunch.Touch");
		EmitSound(filter, pPlayer->entindex(), "Hotlunch.Touch");
...
//---------------------------------------------------------------------------Ч 
// бонус вода, лечит игрока при подн€тии 
//---------------------------------------------------------------------------Ч 
