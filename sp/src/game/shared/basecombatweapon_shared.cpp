...
void CBaseCombatWeapon::EnableIronsights(void)
{
	...
	if (pOwner->SetFOV(this, pOwner->GetDefaultFOV() + GetIronsightFOVOffset(), 0.1f)) //modify the last value to adjust how fast the fov is applied
	{
...
void CBaseCombatWeapon::DisableIronsights(void)
{
	...
	if (pOwner->SetFOV(this, 0, 0.5f)) //modify the last value to adjust how fast the fov is applied
	{
		m_bIsIronsighted = false;
		SetIronsightTime();
